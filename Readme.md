...
### Phase 1: Bug 修复 (4项)
- **IsBotDeafened 震聋机制修复**: `_BotDeafedTime < Time.time` 比较方向反转 → `>`。修复前近距离枪声震聋机制完全失效。
- **对话声音识别修复**: `ProcessSounds` 传入错误列表 → 改为 `AISoundCachedEvents_Conversations`。修复前 AI 完全不对脚步/换弹/语音做出反应。
- **声音定位精度梯度修复**: `ratio` 计算添加括号。修复前运算符优先级错误导致定位精度梯度丧失。
- **NoBushESP 组件标准化**: 基类从 `BotBase` 改为 `BotComponentClassBase`，确保正确的生命周期注册。

### Phase 2: 感知公平性改进 (5项)
- **转向角度梯度误差**: 被击中/压制时转向精度随距离递减（30m 内精确 / 100m 外最大 60°误差），连续被压制逐步提升定位精度。
- **弹着点距离分级误差**: 弹着点定位从恒定误差改为三阶分级（<15m 精确 / 15-80m 曲线膨胀 / 80m+ 极大随机），连续命中缩小定位范围。
- **倍镜视觉分级**: 引入 `GetBotScopeMagnification`，无镜 AI 超 80m 发现速度降至 1/20，8x 镜 AI 有效视距达 350m+。
- **声音定位倍率系数**: 高倍镜狙击手枪声更难精确定位（8x 镜→1.8x 定位难度）。
- **DoorOpener 退后逻辑**: 开门前退后 0.5m，碰撞在门打开 80% 后才移除，减少穿门现象。

### Phase 3: 战斗行为增强 (8项)
- **空仓自动切副武器**: 主武器弹尽时自动切换手枪继续战斗，而非原地罚站换弹。
- **掩体内战术换弹**: 开阔地有敌人时优先找掩体再换弹，掩体内更积极保持满弹匣。
- **小队成员阵亡反应**: 队友死亡时存活成员触发语音反馈 + 行为调整（Coward 撤退/GigaChad 愤怒冲锋）。
- **战后自主恢复行为**: 战斗结束自动检查伤势/弹药，优先治疗再补充弹药。
- **击杀确认与补枪**: 击倒敌人后保持瞄准 2 秒确认击杀状态。
- **受伤跛行移动惩罚**: 腿部骨折→60% 移速、无法冲刺；重伤→75% 移速；决策层禁止 RushEnemy。
- **LootingBots 搜刮威胁中断**: 搜刮时检测到 15m 内敌人/被压制→自动调用 `PreventBotFromLooting` 中断。
- **LootingBots 搜刮前安全就位**: 搜刮前检查掩体状态，无掩体且区域安全时蹲下搜刮，否则阻止搜刮 30s。

### Phase 4: 战术深度与外部整合 (8项)
- **闪光弹战术投掷**: 近距离室内敌人(3-15m)10%概率投掷闪光弹，复用现有手雷逻辑。
- **小队角色自动分配**: 基于武器类型分配狙击/突击/支援角色，影响决策权重。
- **LootingBots 搜刮警戒轮换**: 小队成员搜刮时其他成员自动警戒守望。
- **LootingBots 战后搜刮触发**: 战后恢复完成后自动调用 `ForceBotToScanLoot`。
- **LootingBots 估价缓存**: 物品价格 60s 缓存，减少反射调用开销。
- **兴趣点系统**: 记录战斗/枪声/阵亡位置为兴趣点，和平状态 Bot 探索兴趣点。
- **区域状态标记**: 搜索/危险区域状态记忆，影响后续巡逻行为。
- **冲刺限制逻辑**: 距离<10m/被压制/腿伤/濒死不冲刺。

### Phase 5: 氛围打磨 (3项)
- **战斗疲劳**: 重度压制作为疲劳代理，降低进攻性。
- **敌方武器识别**: 根据武器类型调整 Rush 距离判断（狙击枪不冲/冲锋枪远距可冲）。
- **异常区域警觉**: 战斗中 30s 未听到敌人声音→强制保守决策。

> **总计**: 27 文件（25 修改 + 2 新建）/ +819 行 / -30 行 / 0 编译错误
>
